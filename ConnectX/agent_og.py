...
@jit(nopython=True)
def check_winner0(board, x, y, player, rows, columns, inarow):
...
@jit(nopython=True)
def play_game0(board, move, player, rows, columns, inarow):
    ...
    if check_winner0(board, row, move, player, rows, columns, inarow):
    ...
    while True:
        ...
        if check_winner0(board, random_row, random_move, current_player, rows, columns, inarow):
...
def sim_games0(board, player, rows, columns, inarow):
    ...
    for idx, move in enumerate(valid_moves):
        ...
        for _ in range(num_sims):
            ...
            result = play_game0(board_copy, move, player, rows, columns, inarow)
            if result == player:
                sim_score += 1
            elif result == 0:
                continue
...
def agent(observation, configuration):
    ...
    import time
    t1 = time.time()
    best_move = sim_games0(board, player, configuration['rows'], configuration['columns'], configuration['inarow'])
